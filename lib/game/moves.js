...
function addCardToHand(G, ctx, { card, playerIndex = -1 } = {}) {
    ...
    // checkForBook(G, ctx, { playerIndex })
}

function checkForBook(G, ctx, { playerIndex = -1 }) {
    playerIndex = playerIndex == -1 ? ctx.currentPlayer : playerIndex
    // group by rank
    const values = groupBy(G.hands[playerIndex], 'rank')
    // check for full book
    // const books = values.filter(group => group.length == suitCount)
}
